...
**Data pipeline** (`prepare_data.py`) — Loads every World Cup knockout match since 1930 and builds 10 features out of them. The big ones are Elo rating difference (accounts for 65.6% of the model's decisions), penalty shootout win rates (~34% combined), and then smaller stuff like rolling xG averages, host advantage, and group stage stats.

**Training** (`train_random_forest.py`) — Random Forest with 100 trees, max depth 10, trained on a chronological 80/20 split so future matches don't leak into training. I tried logistic regression (60%) and XGBoost (80%) but Random Forest just worked better.

**Betting odds** (`betting_odds.py`) — I compiled odds from Fox Sports, FanDuel, OddsPortal, and Betfair for 2018, 2022, and 2026 knockout matches. The betting market gets about 83.3% accuracy on the matches I have odds for. The model beats that by ~17 points.

**Interactive CLI** (`interactive_predictor_numbered.py`) — Lists the 9 remaining R32 matches. Pick a number, get a prediction. That's it.
...
Early Round of 32 results validate the model:
- France 3-0 Sweden: Model predicted France 95%+ ✓
- Mexico 2-0 Ecuador: Model predicted Mexico 75% ✓
- Germany 1-1 Paraguay (4-3 pens): Model predicted Germany 70% ✓
...
The 95.5% accuracy isn't just a number—it actually works on real matches.
...
- No player-level data — injuries, suspensions, none of that.
...
- No current form tracking. A team on a hot streak in the tournament gets no credit for that.
...
Built during the 2026 World Cup as a learning project. MIT License.
